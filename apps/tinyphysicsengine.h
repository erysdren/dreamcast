...
/**
  ...
  then by X, then by Y), if not mentioned otherwise. Angles are in floats,
...
#define TPE_INFINITY __builtin_inff()

#define TPE_JOINT_SIZE(joint) ((joint).size)
...
/** Speed, in floats per ticks, that is considered low (used e.g. for auto
  deactivation of bodies). */
  #define TPE_LOW_SPEED 30
#endif

#ifndef TPE_RESHAPE_TENSION_LIMIT
/** Tension limit, in floats, after which a non-soft body will be reshaped.
...
/** Margin, in floats, by which a body will be shifted back to get out of
  collision. */
...
TPE_Vec3 TPE_vec3(float x, float y, float z);
...
TPE_Vec3 TPE_vec3Times(TPE_Vec3 v, float units);
TPE_Vec3 TPE_vec3TimesPlain(TPE_Vec3 v, float q);
TPE_Vec3 TPE_vec3Normalized(TPE_Vec3 v);

float TPE_vec3Dot(TPE_Vec3 v1, TPE_Vec3 v2);
float TPE_vec3Len(TPE_Vec3 v);
float TPE_vec3LenApprox(TPE_Vec3 v);

/** Returns an angle in floats (see angle conventions) of a 2D vector with
  the X axis, CCW. */
float TPE_vec2Angle(float x, float y);

/** Keeps given value within specified range. This can be used e.g. for movement
  smoothing. */
float TPE_keepInRange(float x, float xMin, float xMax);

static inline float TPE_abs(float x);
static inline float TPE_max(float a, float b);
static inline float TPE_min(float a, float b);
static inline float TPE_nonZero(float x);
static inline float TPE_dist(TPE_Vec3 p1, TPE_Vec3 p2);
static inline float TPE_distApprox(TPE_Vec3 p1, TPE_Vec3 p2);

#define TPE_sin(x) __builtin_sinf(x)
#define TPE_cos(x) __builtin_cosf(x)
#define TPE_atan(x) __builtin_atanf(x)
#define TPE_sqrt(x) __builtin_sqrtf(x)
...
typedef TPE_Vec3 (*TPE_ClosestPointFunction)(TPE_Vec3, float);
...
uint8_t TPE_testClosestPointFunction(TPE_ClosestPointFunction f,
  ...
  float allowedError, TPE_Vec3 *errorPoint);
...
static inline float TPE_connectionTension(float length,
  float desiredLength);
...
void TPE_getVelocitiesAfterCollision(float *v1, float *v2, float m1,
  float m2, float elasticity);
...
TPE_Joint TPE_joint(TPE_Vec3 position, float size);
...
uint8_t TPE_jointsResolveCollision(TPE_Joint *j1, TPE_Joint *j2,
  float mass1, float mass2, float elasticity, float friction,
...
uint8_t TPE_jointEnvironmentResolveCollision(TPE_Joint *joint, float
  elasticity, float friction, TPE_ClosestPointFunction env);
...
TPE_Vec3 TPE_castEnvironmentRay(TPE_Vec3 rayPos, TPE_Vec3 rayDir,
  TPE_ClosestPointFunction environment, float insideStepSize,
  float rayMarchMaxStep, uint32_t maxSteps);
...
float TPE_worldGetNetSpeed(const TPE_World *world);
float TPE_bodyGetNetSpeed(const TPE_Body *body);
float TPE_bodyGetAverageSpeed(const TPE_Body *body);
void TPE_bodyMultiplyNetSpeed(TPE_Body *body, float factor);
void TPE_bodyLimitAverageSpeed(TPE_Body *body, float speedMin,
  float speedMax);
...
void TPE_bodyApplyGravity(TPE_Body *body, float downwardsAccel);
...
/** Draws a debug view of a 3D physics world using a provided pixel drawing
  ...
  CamView.z says the camera focal length (~FOV) in floats (0 means
  ...
  environemnt), envGridSize is the size (int floats) of the grid cell. Note
...
void TPE_makeBox(TPE_Joint joints[8], TPE_Connection connections[16],
  float width, float depth, float height, float jointSize);
...
void TPE_makeRect(TPE_Joint joints[4], TPE_Connection connections[6],
  float width, float depth, float jointSize);
void TPE_makeTriangle(TPE_Joint joints[3], TPE_Connection connections[3],
  float sideLength, float jointSize);
...
TPE_Vec3 TPE_envBox(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 maxCornerVec,
  TPE_Vec3 rotation);
TPE_Vec3 TPE_envSphere(TPE_Vec3 point, TPE_Vec3 center, float radius);
TPE_Vec3 TPE_envSphereInside(TPE_Vec3 point, TPE_Vec3 center, float radius);
TPE_Vec3 TPE_envHalfPlane(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 normal);
TPE_Vec3 TPE_envGround(TPE_Vec3 point, float height);
TPE_Vec3 TPE_envInfiniteCylinder(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3
  direction, float radius);
...
TPE_Vec3 TPE_envHeightmap(TPE_Vec3 point, TPE_Vec3 center, float gridSize,
  float (*heightFunction)(int32_t x, int32_t y), float maxDist);
...
TPE_Vec3 TPE_envAATriPrism(TPE_Vec3 point, TPE_Vec3 center,
  const float sides[6], float depth, uint8_t direction);
...
#define TPE_ENV_START(test,point) TPE_Vec3 _pBest = test, _pTest; \
  float _dBest = TPE_DISTANCE(_pBest,point), _dTest; \
...
float TPE_dist(TPE_Vec3 p1, TPE_Vec3 p2)
{
  p1 = TPE_vec3Minus(p1,p2);
  return TPE_vec3Len(p1); 
}

float TPE_distApprox(TPE_Vec3 p1, TPE_Vec3 p2)
{
  p1 = TPE_vec3Minus(p1,p2);
  ...
  for (uint16_t i = 0; i < world->bodyCount; ++i)
  {
    ...
    else // normal, rotating bodies
    {
      ...
      for (uint16_t j = 0; j < body->connectionCount; ++j) // joint tension
      {
        ...
        float tension = TPE_connectionTension(TPE_LENGTH(dir),
          connection->length);
  ...
  float speedMax)
{
  ...
  {
    float speed = TPE_bodyGetAverageSpeed(body);
...
void TPE_bodyCancelOutVelocities(TPE_Body *body, uint8_t strong)
{
  for (uint16_t i = 0; i < body->connectionCount; ++i)
  {
    ...
    float len = TPE_nonZero(TPE_LENGTH(dir));

    uint8_t cancel = 1;

    if (strong)
    {
      float tension = TPE_connectionTension(len,c->length);
...
TPE_Vec3 _TPE_rotateByAxis(TPE_Vec3 p, TPE_Vec3 axisNormalized, float angle)
{
...
void TPE_bodyRotateByAxis(TPE_Body *body, TPE_Vec3 rotation)
{
  ...
  float angle = TPE_LENGTH(rotation);
...
TPE_Vec3 TPE_vec3ProjectNormalized(TPE_Vec3 v, TPE_Vec3 baseNormalized)
{
  ...
  float p = TPE_vec3Dot(v,baseNormalized);
  ...
  TPE_ClosestPointFunction env)
{
  ...
  float d = TPE_LENGTH(dir) - TPE_JOINT_SIZE(*j1) - TPE_JOINT_SIZE(*j2);
  ...
  if (d < 0) // collision?
  {
    ...
    float ratio = (mass2 * TPE_F) /
      TPE_nonZero(mass1 + mass2);

    float shiftDistance = (ratio * d) / TPE_F;
...
void TPE_getVelocitiesAfterCollision(float *v1, float *v2,
  float m1, float m2, float elasticity)
{
  /* In the following a lot of TPE_F cancel out, feel free to
     check if confused. */

  float m1Pm2 = TPE_nonZero(m1 + m2);
  float v2Mv1 = TPE_nonZero(*v2 - *v1);
...
uint8_t TPE_jointEnvironmentResolveCollision(TPE_Joint *joint,
  float elasticity, float friction, TPE_ClosestPointFunction env)
{
  TPE_Vec3 toJoint =
    TPE_vec3Minus(joint->position,env(joint->position,TPE_JOINT_SIZE(*joint)));

  float len = TPE_LENGTH(toJoint);
  ...
  TPE_ClosestPointFunction env)
{
  for (uint16_t i = 0; i < body->jointCount; ++i)
  {
    ...
    float size = TPE_JOINT_SIZE(*joint);
...
void _TPE_vec2Rotate(float *x, float *y, float angle)
{
  float tmp = *x;

  float s = TPE_sin(angle);
  float c = TPE_cos(angle);
...
void _TPE_drawDebugPixel(
  float x, float y, float w, float h, uint8_t c,
...
#define Z_LIMIT 250
  if (world->environmentFunction != 0)
  {
    ...
    float gridHalfSize = (envGridSize * envGridRes) / 2;
...
#undef SEGS
    ...
    for (uint16_t j = 0; j < world->bodies[i].jointCount; ++j)
    {
      ...
      if (p.z > Z_LIMIT)
      {
        ...
        float size = TPE_JOINT_SIZE(world->bodies[i].joints[j]);
...
void TPE_bodyGetAABB(const TPE_Body *body, TPE_Vec3 *vMin, TPE_Vec3 *vMax)
{
  ...
  *vMax = *vMin;
  ...
  float js = TPE_JOINT_SIZE(body->joints[0]);
...
TPE_Vec3 TPE_rotationRotateByAxis(TPE_Vec3 rotation, TPE_Vec3 rotationByAxis)
{
  ...
  float a = TPE_LENGTH(rotationByAxis);
  ...
  float radius)
{
  ...
  float lDir = TPE_nonZero(TPE_LENGTH(direction));

  float lDiff = TPE_LENGTH(projected) - lDir;
  ...
  float radius)
{
  ...
  float l = TPE_sqrt(m.x * m.x + m.z * m.z);

  if (l == 0)
    return TPE_vec3(0,0,0);

  float d = (TPE_DISTANCE(position1,position2) *
    TPE_F) / (radius * 4);
        ...
        for (uint16_t j = 0; j < world->bodies[i].jointCount; ++j)
        {
          ...
          if (TPE_vec3Dot(p,rayDir) >= 0)
          {
            ...
            if (d <= js)
            {
              ...
              float d2 = TPE_DISTANCE(rayPos,i2);
...
TPE_Vec3 TPE_bodyGetLinearVelocity(const TPE_Body *body)
{
  ...
  for (uint16_t i = 0; i < body->jointCount; ++i)
  {
    float *v = body->joints[i].velocity;
...
float TPE_abs(float x)
{
  return x >= 0 ? x : (-1 * x);
}

float TPE_max(float a, float b)
{
  return (a > b) ? a : b;
}

float TPE_min(float a, float b)
{
  return (a < b) ? a : b;
}
  ...
  float (*heightFunction)(int32_t x, int32_t y), float maxDist)
{
  ...
  for (uint16_t i = 0; i < 1024; ++i) // while (1) should work in theory but...
  {
    ...
    for (uint8_t j = 0; j < 2; ++j) // check the two triangles of the segment
    {
      ...
      float testD = TPE_DISTANCE(testP,point);
  ...
  float radius)
{
  ...
  {
    float height = TPE_LENGTH(direction);

    TPE_Vec3 helper = TPE_vec3Project(point,direction);
    float y = TPE_LENGTH(helper);

    helper = TPE_vec3Minus(point,helper);

    float x = TPE_LENGTH(helper);
    ...
    {
      float scaledRadius = radius - ((y * radius) / height);
