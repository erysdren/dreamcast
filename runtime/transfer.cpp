...
void transfer_init_palette(uint32_t type, uint32_t alpha_ref)
{
	...
	switch (type)
	{
		case PALETTE_TYPE_ARGB1555: holly.PAL_RAM_CTRL = holly::pal_ram_ctrl::pixel_format::argb1555; break;
		case PALETTE_TYPE_RGB565: holly.PAL_RAM_CTRL = holly::pal_ram_ctrl::pixel_format::rgb565; break;
		case PALETTE_TYPE_ARGB4444: holly.PAL_RAM_CTRL = holly::pal_ram_ctrl::pixel_format::argb4444; break;
		case PALETTE_TYPE_ARGB8888: holly.PAL_RAM_CTRL = holly::pal_ram_ctrl::pixel_format::argb8888; break;
		default: return;
	}

	holly.PT_ALPHA_REF = alpha_ref;
}

static uint32_t palette_top_index = 0;
...
uint32_t transfer_palette(uint32_t *entries, size_t num_entries)
{
	...
	// palette ram is full
	if (palette_top_index >= 1024)
		return PALETTE_INVALID;

	if (palette_top_index + num_entries > 1024)
		return PALETTE_INVALID;

	for (size_t i = 0; i < num_entries; i++)
	{
		holly.PALETTE_RAM[palette_top_index + i] = entries[i];
	}

	uint32_t palette_index = palette_top_index;

	palette_top_index += num_entries;
